(ns weatherford.main
  "Kotodama WASM entrypoint for the Weatherford clean-room actor (L4) — Clojure port.
  ...
  facts (`weatherford.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "weatherford")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"weatherford.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "weatherford-compat" :tier tier :entities entities} 200])
...
(defn start! [] :weatherford-compat/ready)
